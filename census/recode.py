...
PIPE_TO_SHELL = re.compile(r"\|\s*(sudo\s+)?(sh|bash|zsh|dash)(\s|$)", re.I)
CURL_PIPE_SH = re.compile(
    r"\b(curl|wget)\b[\s\S]{0,200}\|\s*(sudo\s+)?(sh|bash|zsh|dash)\b", re.I
)
ECHO_OUTER = re.compile(r"^\s*(echo|printf)\b", re.I)
SHELL_INVOKE = re.compile(
    r"(?:^|[;&\n]|\|\||&&)\s*(sudo\s+)?(sh|bash|zsh|dash)\b", re.I
)
LOCAL_PATH = re.compile(r"(\.sh\b|\$HOME|\$\{HOME\}|~/|\./|\.cursor/)", re.I)
